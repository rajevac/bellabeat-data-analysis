...
### Cleaning: minuteMETsNarrow_merged.csv

##### Check for data types #####
```{r}
df_minute_METs_merged <- read_csv('raw-data/minuteMETsNarrow_merged.csv')
str(df_minute_METs_merged)
```

Convert ActivityMinute from character to date and time data type
```{r}

df_minute_METs_merged$ActivityMinute <- as.POSIXct(df_minute_METs_merged$ActivityMinute, 
                                                     format = "%m/%d/%Y %I:%M:%S %p")

str(df_minute_METs_merged)
```
```{r}
head(df_minute_METs_merged$ActivityMinute, 24)
```
...
Check for number of rows.
```{r}
nrow(df_minute_METs_merged)
```
Remove duplicate rows, if any, and check for number of rows.
```{r}
df_minute_METs_merged <- distinct(df_minute_METs_merged)
nrow(df_minute_METs_merged)
```
Check for missing values
```{r}
colSums(is.na(df_minute_METs_merged))
```

Save cleaned data
```{r}
write.csv(df_minute_METs_merged,'~/Learning/Data\ Analytics/Google\ Coursera\ Data\ Analytics\ Course/7_Capstone/bellabeat-data-analysis/clean-data/df_minute_METs_clean.csv', row.names = TRUE)
```
